    ...
    def iter_(current_value, depth):
        ...
        for char in current_value:
            if char['type'] == 'add':
                ...
                action = '+ '
                lines.append(
                    f'{new_deep}{action}{char["key"]}: {char["value"]}')
            elif char['type'] == 'del':
                new_deep = deep_indent[:-2]
                action = '- '
                lines.append(
                ...
                action = '- '
                lines.append(
                    f'{new_deep}{action}{char["key"]}: {char["value1"]}')
                new_deep = deep_indent[:-2]
                action = '+ '
                lines.append(
                    f'{new_deep}{action}{char["key"]}: {char["value2"]}')

            elif char['type'] == 'root':
                new_deep = deep_indent
                action = ''
                lines.append(
                    f'{new_deep}{action}{char["key"]}: {iter_(char["value"], deep_indent_size)}')
        ...
        for key in keys:
            if key not in dict2:
                ...
                if isinstance(dict1[key], dict) and isinstance(dict2[key],
                                                               dict):
                    result.append({'type': 'root', 'key': key, 'value': diff_dict(dict1[key], dict2[key])})
                else:
